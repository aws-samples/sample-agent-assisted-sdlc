...
async function deleteGatewayWithTargets(gatewayId) {
  ...
  try {
    ...
    if (targets.length > 0) {
      console.log("Waiting for all targets to be fully deleted...");
      for (let attempt = 0; attempt < 30; attempt++) {
        await sleep(10000);
        const check = await client.send(new ListGatewayTargetsCommand({ gatewayIdentifier: gatewayId }));
        const remaining = (check.targets || []).length;
        console.log(`Attempt ${attempt + 1}: ${remaining} targets remaining`);
