...
#
# DUPLICATE OF project-management/shared/log.py
# Keep in sync with that file. The duplication exists because the runtime
# container Dockerfiles use s3_assets.Asset({path: <runtime-dir>}) which
# limits the build context to the runtime directory, so a COPY ../shared/...
# is not possible. Restructuring to use a parent build context is tracked
# as a follow-up; see the PR body of #23 for the rationale.
