...
2. Call `mcp__gateway__source-control___list_pull_requests` with `head={owner}:feat/issue-{number}` (REST-filtered fast path). If empty or errors, list open PRs and match `headRefName == feat/issue-{number}` client-side (fallback).
3. If a PR is found, call `mcp__gateway__source-control___pull_request_read` and read `state`, `mergedAt`, `mergeStateStatus`. Log `mergeStateStatus` as diagnostic context only — it MUST NOT drive any branching decision.
